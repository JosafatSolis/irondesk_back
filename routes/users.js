...
//ROUTE GET POR ID
router.get("/:id", verifyToken, checkRole(["Tecnician", "Admin"]), (req, res, next) => {
  const { id } = req.params;
  User.findById(id)
    .then((found) => {
      if (found) {
...
// To be used by Tecnicians, overrides role as "User"
router.post("/user", verifyToken, checkRole(["Tecnician", "Admin"]), (req, res) => {
  const { password, ...userValues } = req.body;
  bcrypt.hash(password, 10).then((hashedPassword) => {
    const user = { ...userValues, password: hashedPassword, role: "User" };
    User.create(user)
    .then((created) => {
      if (created) {
        const flatUser = created.toObject();
